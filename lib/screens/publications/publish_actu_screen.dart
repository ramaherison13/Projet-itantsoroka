...
// --- Écran Principal : PublishActuScreen ---

class PublishActuScreen extends StatefulWidget {
  const PublishActuScreen({super.key});

  @override
  State<PublishActuScreen> createState() => _PublishActuScreenState();
}

class _PublishActuScreenState extends State<PublishActuScreen> {
  int currentStep = 1;
  ...
  final List<Map<String, dynamic>> steps = [
    {
      ...
      "description": "Renseignez les informations principales qui décrivent votre actualité.",
    },
    {
      "id": 2,
      "title": "Description et détails",
      "icon": Icons.description_outlined,
      "description": "Décrivez en détail le contenu de l'actualité et ses objectifs.",
    },
    {
      "id": 3,
  ...
  void _handleInputChange(String field, dynamic value) {
    setState(() {
      ...
      } else if (field == 'theme' || field == 'themeId') {
  ...
  bool _validateCurrentStep() {
    if (currentStep == 1) {
      if (formData.title.trim().isEmpty) {
        _showErrorMessage("Le titre de l'actualité est obligatoire");
        return false;
      }
      if (formData.eventType.trim().isEmpty) {
        _showErrorMessage("Le type d'événement est obligatoire");
        return false;
      }
      if (formData.startDate.trim().isEmpty) {
        _showErrorMessage("La date de publication est obligatoire");
        return false;
      }
      if (formData.communeId.trim().isEmpty) {
        _showErrorMessage("Veuillez sélectionner une commune");
        return false;
      }
      if (formData.districtId.trim().isEmpty) {
        _showErrorMessage("Veuillez sélectionner un district");
        return false;
      }
      ...
      try {
        ...
        if (startDate.isBefore(today)) {
          _showErrorMessage("La date de publication ne peut pas être dans le passé");
          return false;
        }
      } catch (_) {
        _showErrorMessage("La date de publication n'est pas valide");
        return false;
      }
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      // Simulation de l'appel API (createEvent)
      await Future.delayed(const Duration(seconds: 2));

      _showSuccessMessage("Actualité publiée avec succès!");
      ...
      _showErrorMessage("Une erreur inattendue s'est produite lors de la publication de l'actualité");
    } finally {
      if (mounted) {
        setState(() {
          isSubmitting = false;
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 8),
                            Text(
                              "Renseignez les informations de votre actualité afin qu'elle soit visible par les lecteurs.",
                      ...
                      const Expanded(
                        child: Text(
                          "Cette section s'adresse aux Communes, CTD et autres acteurs publics souhaitant diffuser des actualités et informations importantes. Publiez vos annonces, communiqués, ou nouvelles ici.",
                ...
                // Main Content Card with Stepper & Form
                Center(
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        // Body Layout (Sidebar steps + Form View)
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            // Sidebar Stepper
                                            ...
                                            children: [
                                              Container(
                                                ...
                                                decoration: BoxDecoration(
                                                  ...
                                                  color: isActive || isCompleted ? Colors.green : Colors.grey.shade300,
                                                ),
                                                child: Center(
                                                  child: Icon(
                                                    step['icon'],
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    if (currentStep == 1) ...[
                                      ...
                                      const SizedBox(height: 16),
                                      TextField(
                                        onChanged: (val) => _handleInputChange('themeId', val),
                                        decoration: const InputDecoration(labelText: "Thème ID", border: OutlineInputBorder()),
                                      ),
                                      const SizedBox(height: 16),
                                      DropdownButtonFormField<String>(
                                      ...
                                      children: [
                                        ...
                                        ElevatedButton(
                                          ...
                                          child: isSubmitting
                                              ...
                                              : Text(currentStep == steps.length ? "Publier l'actualité" : "Suivant"),
                                        ),
                                      ],
                                    ),
                                  ],
          ...
          // Notifications Overlay (Success / Error)
